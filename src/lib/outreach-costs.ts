...
export function callCosts(d: RecordValue | null | undefined): CallCosts {
  ...
  if (models && typeof models === "object" && Object.keys(models).length) {
    const values = Object.values(models) as RecordValue[];
    ...
    if (values.every(v => categories.every(k => number(v?.[k]?.tokens) !== null))) {
      const sum = (key: string) => values.reduce((n, v) => n + v[key].tokens, 0);
