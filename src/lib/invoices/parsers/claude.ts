...
/**
 * The model rung — last on the ladder, not first.
 *
 * Claude reads the pages visually, so scanned and flattened documents work
 * the same as digital ones. It is also the only rung that costs money per
 * call, which is why the text and template rungs run first and why the model
 * is Haiku: the previous parser hardcoded Opus and burned the key's entire
 * credit in a fortnight.
 */
