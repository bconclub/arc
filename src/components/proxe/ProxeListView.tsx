  ...
  return (
    <div className="max-w-dashboard mx-auto px-5 py-6">
      <div className="flex items-center gap-3 mb-1">
        ...
        <button
          onClick={load}
          className="ml-auto flex cursor-pointer items-center gap-2 text-sm text-text-muted hover:text-text transition-colors"
        >
          <RefreshCw className={`w-4 h-4 ${loading ? "animate-spin" : ""}`} /> Refresh
        </button>
      </div>
      <p className="text-sm text-text-muted mb-5">{blurb}</p>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {BRANDS.map((b) => (
          <button
            key={b}
            onClick={() => setBrand(b)}
            className={`cursor-pointer px-3 py-1.5 rounded-full text-sm capitalize transition-all ${
                ...
                return (
                  <div key={b.id} className="border border-border rounded-card overflow-hidden bg-surface">
                    <button
                      ...
                      className="w-full flex cursor-pointer items-center gap-3 px-4 py-3 text-left hover:bg-surface-hover transition-colors"
                    >
